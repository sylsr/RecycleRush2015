...
package src.com.kauailabs.nav6.frc;
...
public class IMU extends SensorBase implements PIDSource, LiveWindowSendable, Runnable {
    ...
    boolean stop = false;
    private src.com.kauailabs.nav6.IMUProtocol.YPRUpdate ypr_update_data;
    protected byte update_type = src.com.kauailabs.nav6.IMUProtocol.MSGID_YPR_UPDATE;
    ...
    public IMU(SerialPort serial_port, byte update_rate_hz) {
        ypr_update_data = new src.com.kauailabs.nav6.IMUProtocol.YPRUpdate();
	...
	int packet_length = src.com.kauailabs.nav6.IMUProtocol.encodeStreamCommand( stream_command_buffer, update_type, update_rate_hz ); 
        try {
    ...
    protected void setStreamResponse( src.com.kauailabs.nav6.IMUProtocol.StreamResponse response ) {
    ...
    public boolean isCalibrating() {
        short calibration_state = (short)(this.flags & src.com.kauailabs.nav6.IMUProtocol.NAV6_FLAG_MASK_CALIBRATION_STATE);
        return (calibration_state != src.com.kauailabs.nav6.IMUProtocol.NAV6_CALIBRATION_STATE_COMPLETE);
    }
    ...
    protected int decodePacketHandler(byte[] received_data, int offset, int bytes_remaining) {
        ...
        int packet_length = src.com.kauailabs.nav6.IMUProtocol.decodeYPRUpdate(received_data, offset, bytes_remaining, ypr_update_data);
        if (packet_length > 0) {
    ...
    public void run() {
        ...
        src.com.kauailabs.nav6.IMUProtocol.StreamResponse response = new src.com.kauailabs.nav6.IMUProtocol.StreamResponse();

        byte[] stream_command = new byte[256];
        
	int cmd_packet_length = src.com.kauailabs.nav6.IMUProtocol.encodeStreamCommand( stream_command, update_type, update_rate_hz ); 
        try {
            serial_port.reset();
            ...
            try {
                ...
                if (bytes_read > 0) {
                    ...
                    while (i < bytes_read) {
                        ...
                        {
                            packet_length = src.com.kauailabs.nav6.IMUProtocol.decodeStreamResponse(received_data, i, bytes_remaining, response);
                            if (packet_length > 0) {
                                packets_received++;
                    ...
                    if ( !stream_response_received && ((Timer.getFPGATimestamp() - last_stream_command_sent_timestamp ) > 3.0 ) ) {
                        cmd_packet_length = src.com.kauailabs.nav6.IMUProtocol.encodeStreamCommand( stream_command, update_type, update_rate_hz ); 
                        try {
                            last_stream_command_sent_timestamp = Timer.getFPGATimestamp();
                            serial_port.write( stream_command, cmd_packet_length );
                            serial_port.flush();
